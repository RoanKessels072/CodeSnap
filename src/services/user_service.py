...
def sync_user_from_keycloak(db: Session, keycloak_id: str, username: str = None, email: str = None):
    """
    Sync a user record from Keycloak when they log in.
    If the user doesn't exist, create them.
    If they exist, update their last login time and username/email if changed.
    """
...
def get_or_create_user(db: Session, keycloak_id: str, username: str, email: str = None):
    """
    ...
    if user:
        return sync_user_from_keycloak(db, keycloak_id, username, email)
    else:
        return sync_user_from_keycloak(db, keycloak_id, username, email)
