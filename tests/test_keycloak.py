...
import jwt
from middleware.keycloak_auth import (
    get_jwks_client, verify_token, sync_user_from_token,
    get_user_info_from_token, require_auth, optional_auth
)
...
class TestKeycloakAuth:
    def test_get_jwks_client_singleton(self):
        with patch('middleware.keycloak_auth.PyJWKClient') as mock_client:
            ...
            import middleware.keycloak_auth
            middleware.keycloak_auth._jwks_client = None
    ...
    class TestKeycloakAuth:
        def test_sync_user_from_token_new_user(self, patch_db_session):
            decoded_token = {
                'sub': 'new-keycloak-id',
    ...
    def test_sync_user_from_token_missing_sub(self):
        decoded_token = {'preferred_username': 'user'}
